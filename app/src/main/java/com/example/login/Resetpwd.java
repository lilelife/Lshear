...
import com.example.Services.ResetpwdToServer;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case SendDateToLogin.SEND_SUCCESS:
                    Toast.makeText(Resetpwd.this, "登录成功", Toast.LENGTH_SHORT).show();
                    break;
                case SendDateToLogin.SEND_FAIL:
                    Toast.makeText(Resetpwd.this, "登录失败", Toast.LENGTH_SHORT).show();
                    break;
        ...
        if (isUserNameAndPwdValid()) {
            ...
            if(result==1){                                             //返回1说明用户名和密码均正确,继续后续操作
                if(userPwd_new.equals(userPwdCheck)==false){           //两次密码输入不一样
                    ...
                    UserData mUser = new UserData(userName, userPwd_new); //xi建的user
                    ...
                    if (flag == false) {
                        ...
                        new ResetpwdToServer(handler).resetpwdToServer(userName,userPwd_old,userPwd_new);////  将新密码在服务器中修改
